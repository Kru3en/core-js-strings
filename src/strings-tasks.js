...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, (char) => {
    const shiftedCharCode = char.charCodeAt(0) + 13;
    return String.fromCharCode(
      shiftedCharCode <= (char <= 'Z' ? 90 : 122)
        ? shiftedCharCode
        : shiftedCharCode - 26
    );
  });
...
function getCardId(value) {
  ...
  const suitValue = suits.indexOf(value.slice(-1)) * 13;
  const rankValue = ranks.indexOf(value.slice(0, -1));
